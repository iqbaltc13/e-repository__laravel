...
return new class extends Migration
{
    ...
    public function up(): void
    {
        $this->schema->create($this->table_name, function (Blueprint $table) {
            ...
            $table->enum('status', ['draft', 'submitted', 'under_review', 'accepted', 'published', 'rejected'])->default('submitted')->nullable();
            $table->enum('thesis_type', ['undergraduate (S1)', 'masters (S2)', 'doctoral (S3)'])->default('undergraduate (S1)')->nullable();
            $table->text('pdf_file')->nullable();
            $table->text('other_document_file')->nullable();
